  ...
  return (0, import_react13.useEffect)(() => {
  ...
  }, []), (0, import_react11.useEditor)(
    {
      ...
      onSelectionUpdate: ({ editor: editor2 }) => {
        ...
        }), !editor2?.isActive("suggestion") && !editor2?.isActive("post") && !firsttime && (param.get("with") === "Suggestion" || param.get("with") === "Post") && setSearchParams((p) => (p.delete("thread"), p.set("with", "all"), p)), firsttime = !1;
      },
...
var updateSourceAndTranslation = async ({
  sourceId,
  sourceContent,
  sourceAnnotation,
  translationId,
...
// app/routes/text_.$textId.page.$pageId.translation.$translationId.tsx
var import_bs = require("react-icons/bs"), import_ai2 = require("react-icons/ai"), import_flowbite_react4 = require("flowbite-react"), import_io3 = require("react-icons/io"), import_react17 = require("react"), import_react_hot_toast3 = __toESM(require("react-hot-toast"));
...
// app/lib/translationhtmlparser.ts
function processHTML(htmlString) {
  let body = new DOMParser().parseFromString(htmlString, "text/html").body, annotations = [], textContent = "";
  ...
  function walkNodes(node, index = 0, openTags = []) {
    return Array.from(node.childNodes).forEach((child) => {
      if (child.nodeType === Node.TEXT_NODE) {
        let content = child.textContent, end = index + content.length;
        textContent += content, openTags.forEach((tag) => {
          tag.end = end;
        }), index = end;
        ...
        let tagAnnotation = {
          ...
          // Temporary end value, will be updated
          content: ""
        };
        openTags.push(tagAnnotation), annotations.push(tagAnnotation), index = walkNodes(child, index, openTags), openTags.pop();
      }
    }), index;
  }
...
var import_jsx_dev_runtime15 = require("react/jsx-dev-runtime"), loader2 = async ({ request, params }) => {
  ...
  let formdata = await request.formData(), sourceContent = formdata.get("sourceContent"), translationContent = formdata.get("translationContent"), sourceAnnotation = JSON.parse(formdata.get("sourceAnnotation")), translationAnnotation = JSON.parse(formdata.get("translationAnnotation")), sourceId = formdata.get("sourceId"), translationId = formdata.get("translationId");
  try {
    let [userText, translation] = await updateSourceAndTranslation({
      sourceContent,
      sourceAnnotation,
      sourceId,
      translationContent,
...
function TranslationsRoute() {
  ...
  function save() {
    ...
    let sourceContent = source_editor.getHTML(), translationContent = translation_editor.getHTML(), source = processHTML(sourceContent), translation2 = processHTML(translationContent), sourceText = source.text, sourceAnnotation = source.annotations, translationText = translation2.text, translationAnnotation = translation2.annotations;
...
function EditorMenu({ editor, selection, setSelectionRange, setSearchParams, isPostAllowed }) {
  ...
  }, InSelectionLimit = selection.content.length > 0 && selection.content.length < 239, isTextAdmin = text.userId !== user?.id;
  return /* @__PURE__ */ (0, import_jsx_dev_runtime20.jsxDEV)(import_jsx_dev_runtime20.Fragment, { children: editor && user && /* @__PURE__ */ (0, import_jsx_dev_runtime20.jsxDEV)(
...
var import_jsx_dev_runtime46 = require("react/jsx-dev-runtime"), loader3 = async ({ request, params }) => {
  ...
  let text = await getText(textId), page = await getPage(textId, order, version), annotations = await getAnnotations(page?.id), user = await getUserSession(request), translations = searchParamsWith === "Translations" ? await listTranslations(textId, page?.id) : [], suggestions = searchParamsWith === "Suggestion" ? await findAllSuggestionByPageId(page?.id, thread) : [], posts = searchParamsWith === "Post" ? await findPostByTextIdAndPage(textId, order, version) : [];
...
function ContentArea({ latestTexts, isLoading }) {
  ...
  return /* @__PURE__ */ (0, import_jsx_dev_runtime49.jsxDEV)("div", { className: "flex w-full mx-auto max-w-2xl flex-col gap-3 items-center justify-start py-10", children: [
