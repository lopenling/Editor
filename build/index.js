...
var import_jsx_dev_runtime29 = require("react/jsx-dev-runtime"), loader3 = async ({ request, params }) => {
  let textId = params.textId, order = params.pageId, text = await getText(textId), page = await getPage(parseInt(textId), parseInt(order)), user = await getUserSession(request), suggestions = await findAllSuggestionByPageId(page == null ? void 0 : page.id);
