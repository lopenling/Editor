...
import { openFilterState, selectedPostThread as selectedPostThreadState, showLatest, showPostContent } from '~/states';
...
export default function PostContainer() {
  ...
  return (
    <>
      <div className="sticky top-0 mb-4 flex w-full  items-center justify-between gap-2 bg-white z-50">
        <div className="ml-2 flex items-center gap-2 0 z-30 py-2" >
    ...
    <Dropdown
      label={
        <>
          ...
          <span className="sort ml-2 text-sm font-medium leading-tight text-gray-500 dark:text-gray-50">Sort By</span>
        </>
      }
      size="sm"
      color="gray-500 dark:text-gray-50"
    >
      <DropdownItem active={isLatestPost} onClick={() => setIsLatestPost(true)}>
        Latest
      </DropdownItem>
      <DropdownItem active={!isLatestPost} onClick={() => setIsLatestPost(false)}>
        Earliest
      </DropdownItem>
    </Dropdown>
  );
